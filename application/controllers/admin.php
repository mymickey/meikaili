...
class Admin extends CI_Controller {
	...
	{
		$data['img_url_prefix'] = $this->config->item('sae_img_storage_prefix');
	...
	public function delete_item(){
        $storeDomain = $this->config->item('sae_storageImgDomain'); 
		$this->M_item->delete_item($storeDomain,$_POST['item_id'],$_POST['file_name']);
	}
	...
	public function saveimage(){
        ...
        try{
		 $this->M_item->save_image($image_source_url,$image_new_name,$storeDomain);
        }
        catch(Exception $e)
          {
               //输出500错误表示保存图片失败
