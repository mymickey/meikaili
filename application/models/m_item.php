...
class M_item extends CI_Model{
	...
	function delete_item($storeName,$id,$fileName){
		$data = array(
               'id' => $id
            );
		$this->db->delete('item', $data);
        $this->delete_item_image($storeName ,$fileName);
		echo '1';
	}
    function delete_item_image($storeName ,$fileName){
        $s = new SaeStorage();
        $s->delete($storeName,$fileName);
    }
	/*
	  * 通过条目ID获得点击url
	   */
     ...
     * @return string 图片保存名
     */
	function save_image($image_source_url,$image_new_name,$store){
        ...
        if($type){
            return resizeImage($src_im,230,230,'images/',$image_new_name,$type,$store);
        }
        else{
